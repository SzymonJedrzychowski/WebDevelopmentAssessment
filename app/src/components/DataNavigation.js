    ...
    return (
        <>
            <Form.Group className="pageLimitSelection">
                ...
                <Form.Select aria-label="Default select example" onChange={props.setPageLimit}>
            ...
            <Form.Group className="pageButtons">
                {(currentPage > 0) && <Button onClick={props.setCurrentPage} value={0}>{"<<"}</Button>}
                {(currentPage <= 0) && <Button disabled>{"<<"}</Button>}
                {(currentPage + 1 >= maxPageNumber && currentPage - 2 >= 0) && <Button onClick={props.setCurrentPage} value={currentPage - 2}>{currentPage - 1}</Button>}
                {currentPage - 1 >= 0 && <Button onClick={props.setCurrentPage} value={currentPage - 1}>{currentPage}</Button>}
                ...
                {currentPage + 1 < maxPageNumber && <Button onClick={props.setCurrentPage} value={currentPage + 1}>{currentPage + 2}</Button>}
                {(currentPage - 1 < 0 && currentPage + 2 < maxPageNumber) && <Button onClick={props.setCurrentPage} value={currentPage + 2}>{currentPage + 3}</Button>}
                {(currentPage < maxPageNumber - 1) && <Button onClick={props.setCurrentPage} value={maxPageNumber - 1}>{">>"}</Button>}
                {(currentPage >= maxPageNumber - 1) && <Button disabled>{">>"}</Button>}
            </Form.Group>

            <Form.Group className="jumpToPage">
                <Form.Label>Jump to page:</Form.Label>
                <Form.Select aria-label="Default select example" onChange={props.setCurrentPage}>
